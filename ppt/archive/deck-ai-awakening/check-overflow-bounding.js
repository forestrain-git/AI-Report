  ...
  for (const f of slides) {
    ...
    const result = await page.evaluate(() => {
      ...
      for (const el of elems) {
        ...
        if (rect.bottom > 1080 + 1) {
          issues.push({ tag: el.tagName, class: el.className, id: el.id, bottom: Math.round(rect.bottom), right: Math.round(rect.right) });
        }
        if (rect.right > 1920 + 1) {
          issues.push({ tag: el.tagName, class: el.className, id: el.id, bottom: Math.round(rect.bottom), right: Math.round(rect.right) });
        }
      }
      return issues;
    ...
    if (result.length > 0) {
      overflows.push({ name, issues: result.slice(0, 5) });
      await page.screenshot({ path: `overflow_${name.replace('.html','')}.png`, fullPage: false });
    }
  }

  console.log('=== OVERFLOW REPORT (Bounding Box) ===');
  if (overflows.length === 0) {
    console.log('All 39 slides fit within 1920x1080.');
  } else {
    for (const o of overflows) {
      console.log(`\n${o.name}: ${o.issues.length} overflowing elements`);
      for (const i of o.issues) {
        console.log(`  <${i.tag}> class="${i.class}" id="${i.id}" bottom=${i.bottom} right=${i.right}`);
      }
    }
  }
  await browser.close();
